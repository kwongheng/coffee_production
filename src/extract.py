...
def extract_from_kaggle(kaggle_path, file_name, new_file_name = ''):
    ...
    print(f'saving file to: {DEFAULT_TARGET_DIR}{file_to_save}')
    df.to_csv(DEFAULT_TARGET_DIR + file_to_save, index=False)
...
def extract_from_url(url, file_name, storage_options):
    ...
    print(f'saving file to: {DEFAULT_TARGET_DIR}{file_name}')
    df.to_csv(DEFAULT_TARGET_DIR + file_name, index=False)
